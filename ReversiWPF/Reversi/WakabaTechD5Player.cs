...
namespace ReversiEngine.Reversi {
    ///<summary>吉村プレイヤー (ゲーム木の深さ 5)</summary>
    public class WakabaTechD5Player : Player {
        Stone player_color;
        Evaluator evaluator;
        
        /// <summary>コンストラクタ</summary>
        /// <param name="player_color">石の色</param>
        public WakabaTechD5Player(Stone player_color) {
        ...
        public Decision Play(Board current_board) {
            if(current_board.CountStone(Stone.None) > 10) {
                ...
                return IterativeDeepeningDepthFirstSearchMethod<State, Decision>.Search(root_state, 5, Decision.Pass); 
            }
            else {
